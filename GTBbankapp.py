...
def welcome():
    time.sleep(1)
    print("Redirectin to GTB...")
    time.sleep(2)
    print("You are welcome to GTBank\n Enter 1 to login or 2 to create new account")
    choice = input(">>> ")
    if choice =="1":
        login()
    elif choice == "2":
...
def registration():
        ...
        querry= "INSERT INTO gtb (First_name, Middle_name, Last_name, Gender, Age, Email, BVN, NIN,acc_nu, Balance, Pass_word , Pin) VALUES (%s, %s, %s, %s, %s, %s, %s, %s,%s, %s,%s, %s)"
        for holder in range(12):
            if bank_info[holder] == "BVN":
                user = random.randint(5460270000, 5460299999)
            elif bank_info[holder]== "NIN":
                user= random.randint( 2130121000, 2130129876)
            elif bank_info[holder]=="Balance":
                 user = 0
            elif bank_info[holder]=="acc_nu":  
                user= random.randint(3121043000, 3121049999)
            else:    
...
def login():
    ...
    querry = "select * from gtb where Email=%s and Pass_word=%s "
    cursor.execute(querry, val)
    global result
...
def transfer_GTB():
        ...
        if resultWE:
            ...
            if request3 == pin:
                ...
                querry2="UPDATE gtb SET Balance=%s where Email=%s"
